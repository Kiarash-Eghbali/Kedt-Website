	...
	return (
		<>
			<div className={`group flex items-center justify-center flex-col w-14 md:w-17 lg:w-20 `}>
				<div className={`  ${className ? className : ` group-hover:-translate-y-2.5 transition-all duration-150 ${isDark ? "bg-[#252525]" : "group-hover:bg-white"} w-14 h-14 md:w-17 md:h-17 lg:w-20 lg:h-20 flex items-center justify-center rounded-lg `} `}>
				...
				<h1 className={`text-xl ${poppins.className} text-transparent ${isDark ? "group-hover:text-white" : "group-hover:text-[#252525]"} transition-all duration-150 ${classLabel ? classLabel : ""} select-none`}>{name}</h1>
			</div>
		</>
	);
}
